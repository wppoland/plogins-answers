...
{
    private const META   = 'answers_pro_banner_dismissed';
    private const ACTION = 'answers_dismiss_pro';
    ...
    {
        if ($this->data === null) {
            $file = ANSWERS_DIR . 'config/pro-upsell.php';
    ...
    public function enabled(): bool
    {
         ...
         */
        return (bool) apply_filters('answers/show_pro_cta', true) && $this->features() !== [];
    }

    private function url(): string
    {
        $default = (string) ($this->data()['url'] ?? 'https://plogins.com/plogins-answers-pro/pricing/');
        /**
         * Filters the URL the "Upgrade to PRO" buttons point at.
         *
         * @param string $url Default the Answers PRO pricing page.
         */
    ...
    private function priceLabel(): string
    {
        ...
        if ($this->isPolish() && ! empty($d['price_pln'])) {
            ...
            return sprintf(__('od %d zł/rok', 'plogins-answers'), (int) $d['price_pln']);
        }
    ...
    public function handleDismiss(): void
    {
        ...
        wp_safe_redirect(wp_get_referer() ?: admin_url('admin.php?page=answers-settings'));
        exit;
    }
    ...
    public function banner(): void
    {
        ...
        ?>
        <div class="answers-pro-banner" role="note">
            <span class="answers-pro-banner__tag">PRO</span>
            ...
            <a class="button button-primary answers-pro-banner__cta" href="<?php echo esc_url($this->url()); ?>" target="_blank" rel="noopener noreferrer">
                <?php esc_html_e('Upgrade to PRO', 'plogins-answers'); ?>
            </a>
        ...
        <aside class="answers-card answers-pro-aside" aria-labelledby="answers-pro-aside-h">
            ...
            <ul class="answers-pro-aside__list">
                <?php foreach ($features as $f) : ?>
                    <li>
                        <span class="answers-pro-aside__lock" aria-hidden="true"></span>
                        <span><?php echo esc_html($f['title']); ?></span>
                    </li>
                <?php endforeach; ?>
            </ul>
            <a class="button button-primary button-hero answers-pro-aside__cta" href="<?php echo esc_url($this->url()); ?>" target="_blank" rel="noopener noreferrer">
                <?php esc_html_e('Upgrade to PRO', 'plogins-answers'); ?>
            </a>
            <?php if ($price !== '') : ?>
                <p class="answers-pro-aside__price"><?php echo esc_html($price); ?> · <?php esc_html_e('one licence, every PRO feature', 'plogins-answers'); ?></p>
            <?php endif; ?>
        </aside>
        <?php
    }
    ...
    public function cards(): void
    {
        ...
        <section class="answers-pro-cards" aria-labelledby="answers-pro-cards-h">
            ...
            <div class="answers-pro-cards__grid">
                <?php foreach ($features as $f) : ?>
                    <article class="answers-pro-card">
                        <span class="answers-pro-card__badge">PRO</span>
                        <span class="answers-pro-card__lock" aria-hidden="true"></span>
                        <h3 class="answers-pro-card__title"><?php echo esc_html($f['title']); ?></h3>
                        <?php if ($f['desc'] !== '') : ?>
                            <p class="answers-pro-card__desc"><?php echo esc_html($f['desc']); ?></p>
                        <?php endif; ?>
                    </article>
                <?php endforeach; ?>
            </div>
